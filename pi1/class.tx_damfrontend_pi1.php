...
class tx_damfrontend_pi1 extends tslib_pibase
{
	...
	function init($conf)
	{
		...
		//
		if ($this->conf['catMounts'] == 'USER') {
			$this->conf['catMounts'] = $this->cObj->USER($this->conf['catMounts.'], '');
		}
		if ($this->conf['catPreSelection'] == 'USER') {
			$this->conf['catPreSelection'] = $this->cObj->USER($this->conf['catPreSelection.'], '');
		}
		if ($this->conf['uploadMounts'] == 'USER') {
			$this->conf['uploadMounts'] = $this->cObj->USER($this->conf['uploadMounts.'], '');
		}
		if ($this->conf['catPreSelection'] == 'USER_INT') {
			$this->conf['catPreSelection'] = $this->cObj->USER($this->conf['catPreSelection.'], 'INT');
		}
		if ($this->conf['uploadMounts'] == 'USER_INT') {
			$this->conf['uploadMounts'] = $this->cObj->USER($this->conf['uploadMounts.'], 'INT');
		}
		if ($this->conf['catMounts'] == 'USER') {
			$this->conf['catMounts'] = $this->cObj->USER($this->conf['catMounts.'], 'USER');
		}
 ...
 *  */
	...
	function getIncomingDocData()
	{
		...
		if ($this->conf['upload.']['autoAssignFEGroups']) {
			$tempArr = array();
			if ($this->conf['upload.']['autoAssignFEGroups'] == 'USER_INT') {
				$this->conf['upload.']['autoAsignFEGroups'] = $this->cObj->USER($this->conf['upload.']['autoAssignFEGroups.'], 'INT');
			}
			if ($this->conf['upload.']['autoAssignFEGroups'] == 'USER') {
				$this->conf['upload.']['autoAssignFEGroups'] = $this->cObj->USER($this->conf['upload.']['autoAssignFEGroups.'], 'USER');
			}
